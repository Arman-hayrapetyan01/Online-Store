...
const ElectonicPage = () => {
  const electroInfo = useSelector((state) => state.allProducts.products);
  const electroData = [];
  document.title = "Midle Store - Electronic Products";
  const dispatch = useDispatch();
  for (let i = 0; i < electroInfo.length; i++) {
    if (electroInfo[i].category === "electronics") {
      electroData.push(electroInfo[i]);
    }
  }
  useEffect(() => {
    window.scrollTo(0, 0);
