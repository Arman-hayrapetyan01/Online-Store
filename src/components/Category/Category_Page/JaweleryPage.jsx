...
const JweleryPage = () => {
  const jeweleryInfo = useSelector((state) => state.allProducts.products);
  const jeweleryData = [];
  document.title = "Midle Store - Jewelery Products";
  const dispatch = useDispatch();
  for (let i = 0; i < jeweleryInfo.length; i++) {
    if (jeweleryInfo[i].category === "jewelery") {
      jeweleryData.push(jeweleryInfo[i]);
    }
  }
  useEffect(() => {
    window.scrollTo(0, 0);
