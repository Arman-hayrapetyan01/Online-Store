...
const WoomensPage = () => {
  const woomensInfo = useSelector((state) => state.allProducts.products);
  const woomensData = [];
  document.title = "Midle Store - Womens Clothing Products";
  const dispatch = useDispatch();
  for (let i = 0; i < woomensInfo.length; i++) {
    if (woomensInfo[i].category === "women's clothing") {
      woomensData.push(woomensInfo[i]);
    }
  }
  useEffect(() => {
    window.scrollTo(0, 0);
